...
client.on('roleDelete', async (role) => {
  try {
    ...
    const deleter = auditLogs?.entries.first()?.executor || role.guild.me;
    ...
    try {
      ...
      const now = new Date();
      console.log(`[ROLE_DELETE] actionTime: ${actionTime}`);
...
client.on('channelCreate', async (channel) => {
  ...
  try {
    ...
    const creator = auditLogs?.entries.first()?.executor || channel.guild.me;
...
client.on('channelDelete', async (channel) => {
  ...
  try {
    ...
    const deleter = auditLogs?.entries.first()?.executor || channel.guild.me;

    console.log(`[CHANNEL_DELETE] Channel: ${channel.name}`);
    console.log(`[CHANNEL_DELETE] Deleter: ${deleter.tag} (${deleter.id})`);
    console.log(`[CHANNEL_DELETE] Bot ID: ${channel.guild.me.id}`);
    ...
    if (auditLogs?.entries.first()) {
      ...
      console.log(`[CHANNEL_DELETE] Audit entry target: ${entry.target?.tag}`);
      console.log(`[CHANNEL_DELETE] Audit entry time: ${entry.createdAt}`);
    }
    ...
    // ============ PUNISHMENT: Remove roles only (NO KICK) ============
    // Check if deleter is not bot
    if (deleter.id === channel.guild.me.id) {
      console.log('[CHANNEL_DELETE] Skipped: deleter is bot');
      return;
    }
    ...
    try {
      ...
      const actionTime = auditLogs?.entries.first()?.createdAt;
      const now = new Date();
      console.log(`[CHANNEL_DELETE] actionTime: ${actionTime}`);
